...
results_dir = Path("results") / "tissue_clocks_revision" / "associations" / "morbidity"
...
fig1, axes1 = plt.subplots(1, 4, figsize=(4 * 2, 2), sharey=True)
fig2, axes2 = plt.subplots(1, 4, figsize=(4 * 3, 5), sharey=True)
for organ, ax1, ax2 in zip(organs, axes1, axes2):
    p = (
        fits.query("metric == @metric & Organ == @organ")
        .pivot_table(
            columns=["pathology"],
    ...
    g = clustermap(
        p["Coef."].drop("n_comorbidities", axis=1),
        figsize=(8, 4),
        cmap="vlag",
        center=0,
