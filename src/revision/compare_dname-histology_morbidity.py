...
results_dir = (
    Path("results") / "tissue_clocks_revision" / "dname_comparison" / "morbidity"
)
results_dir.mkdir(parents=True, exist_ok=True)
figkws = dict(dpi=300, bbox_inches="tight")

meta = pd.read_csv(data_dir / "GTEx Portal.csv", index_col=0)
meta["Organ"] = meta["Tissue"].str.replace(r" - .*", "", regex=True)
...
t = feats.loc[:, feats.columns.str.startswith("morbidity")].astype(bool)
n = t.sum(0).rename("n_individuals").sort_values()
t = t.loc[:, n >= 3].astype(float)
t = t.sum(1).rename("value").to_frame()
t = meta.merge(t, left_on="Subject ID", right_index=True, how="left")[
    ["value", "Organ"]
]
t["CollaboratorParticipantID"] = t.index.str.extract(r"(GTEX-\w+)-\d{4}")[0].values
...
for run in runs:
    ...
    fits.to_csv(
        results_dir / f"{modality}.pathology_all_models.{metric}.stats.csv",
        index=False,
    )
    ...
    fig.tight_layout()
    fig.savefig(
        results_dir / f"{modality}.pathology_all_models.{metric}.stats.heatmap.svg",
        **figkws,
    )
    ...
    fig.tight_layout()
    fig.suptitle("Morbidities vs Age residuals")
    fig.savefig(
        results_dir / f"{modality}.pathology_all_models.{metric}.stats.barplot.svg",
        **figkws,
    )
