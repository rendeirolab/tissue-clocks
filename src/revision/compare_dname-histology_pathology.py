...
results_dir = (
    Path("results") / "tissue_clocks_revision" / "dname_comparison" / "pathology"
)
...
t = (
    get_pathology_data()
    .drop(["spermatogenesis"], axis=1)
    .sum(axis=1)
    .rename("value")
    .to_frame()
)
t["Organ"] = meta["Organ"].reindex(t.index)
...
for run in runs:
    ...
    fig.tight_layout()
    fig.suptitle("Pathologies vs Age residuals")
    fig.savefig(
