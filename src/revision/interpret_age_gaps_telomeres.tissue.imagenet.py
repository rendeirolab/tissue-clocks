...
results_dir = (
    Path("results") / "tissue_clocks_revision" / "gtex_imagenet_clocks" / "telomeres"
)
    ...
    for metric, axs in zip(["Age", "pred", "residual"], axes.T):
        ...
        for organ, ax in zip(organs, axs):
            try:
                ...
                continue
            f = input_dir / f"ridgecv_{suffix}.{organ}.predictions.csv"
            if not f.exists():
                continue
...
fits = fits.query("Organ.isin(@organs)")
